...
final class EditVideoViewController: UIViewController {
    ...
    // MARK: Public
    var onFinish: ((URL, CMTimeRange) -> Void)?
    ...
    private func setExporting(_ exporting: Bool) {
        var cfg = continueButton.configuration
        cfg?.showsActivityIndicator = exporting
        let attrs = AttributeContainer([.font: UIFont.systemFont(ofSize: 18, weight: .semibold),
                                        .foregroundColor: UIColor.systemBackground])
        cfg?.attributedTitle = AttributedString(exporting ? "Exporting…" : "Continue", attributes: attrs)
        cfg?.baseBackgroundColor = exporting ? .tertiaryLabel : (continueButton.isEnabled ? .label : .tertiaryLabel)
        continueButton.configuration = cfg
        view.isUserInteractionEnabled = !exporting
    }

    private func exportTrimmedVideo(range: CMTimeRange, completion: @escaping (URL?) -> Void) {
        guard let asset = self.asset else { completion(nil); return }
        // Prefer pass-through; fallback to highest quality if needed
        ...
        // Pick a supported file type (prefer .mp4, then .mov, then anything)
        let preferredTypes: [AVFileType] = [.mp4, .mov, .m4v]
        let type = preferredTypes.first(where: { exporter.supportedFileTypes.contains($0) }) ?? exporter.supportedFileTypes.first
        guard let fileType = type else { completion(nil); return }

        let ext: String = (fileType == .mp4 ? "mp4" : fileType == .mov ? "mov" : "m4v")
        let outURL = FileManager.default.temporaryDirectory.appendingPathComponent("coachai-trimmed-\(UUID().uuidString).\(ext)")
        ...
        exporter.exportAsynchronously { [weak self] in
            DispatchQueue.main.async {
                guard exporter.status == .completed else {
                    print("Export failed:", exporter.error ?? NSError(domain: "export", code: -1))
                    completion(nil)
                    return
                }
                completion(outURL)
                // keep self alive not required
            }
        }
    }
    ...
    private func didTapContinue() {
        guard let start = trimmer.startTime, let end = trimmer.endTime else { return }
        let range = CMTimeRange(start: start, end: end)
        // Validity is already enforced by the disabled button; still guard:
        let sel = CMTimeGetSeconds(range.duration)
        guard sel > 0, sel <= maxDuration else { return }

        setExporting(true)
        exportTrimmedVideo(range: range) { [weak self] url in
            guard let self = self else { return }
            self.setExporting(false)
            guard let trimmedURL = url else {
                // Optional: show a quick alert
                let a = UIAlertController(title: "Export Failed", message: "Couldn’t create trimmed clip. Try again.", preferredStyle: .alert)
                a.addAction(UIAlertAction(title: "OK", style: .default))
                self.present(a, animated: true)
                return
            }
            self.onFinish?(trimmedURL, range)
        }
    }
}
